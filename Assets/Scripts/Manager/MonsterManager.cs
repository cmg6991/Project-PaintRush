...
public class MonsterManager : MonoBehaviour {
    ...
    public bool RegisterDeath(MonsterAI monster) {
        ...
        if (!aliveMonsters.Remove(monster)) { return false; }
        ...
        Debug.Log( $"몬스터 처치 수: {killCount}, 생존 몬스터 수: {aliveMonsters.Count}" );
    ...
    public int DamageAll(
        ...
        Color attackColor,
        GameObject attacker,
        bool ignoreElement) { if (damage <= 0) { return 0; }

        MonsterAI[] targets = GetAliveMonstersSnapshot();
        int attackedCount = 0;

        foreach (MonsterAI monster in targets) { if (monster == null || monster.IsDead) { continue;  }
    ...
    public MonsterAI[] GetAliveMonstersSnapshot() {
        aliveMonsters.RemoveWhere( monster => monster == null || monster.IsDead );
